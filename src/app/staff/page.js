...
export default function StaffAccessHub() {
  ...
  useEffect(() => {
    ...
    // GAS မှ fresh permissions ယူမည်
      ...
      .then(res => {
        if (res.success && res.data) {
          const fresh = res.data.find(s =>
            ...
            (s.Username && s.Username === auth.username)
          );
          if (fresh) {
  ...
  const toolGroups = [
    {
      ...
      items: [
        ...
        { name: 'House Score Adjust', path: '/staff/points', icon: '⚖️', locked: !hasPermission('Can_Record_Points') },
      ]
    },
    {
      ...
      items: [
        ...
        { name: 'My Leave', path: '/staff/my-leave', icon: '📄', locked: false },
      ]
    },
    {
