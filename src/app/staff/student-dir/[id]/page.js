...
export default function StudentDetailPage() {
  ...
  const getHouseTheme = (houseName) => {
    ...
    return { bg: 'linear-gradient(135deg,#4c1d95,#1e1b4b)', textColor:'#FFD700', badgeBg:'#FFD700' };
  };
  ...
  if (loading) return <div className="min-h-screen bg-slate-50 flex items-center justify-center font-black animate-pulse text-2xl uppercase italic px-6" style={{color:'#4c1d95'}}>Loading Profile...</div>;
      ...
      <div className="mx-auto space-y-6" style={{maxWidth:'1400px'}}>
        ...
        <div className="flex flex-col sm:flex-row justify-between items-center px-2 md:px-4 pt-2 pb-4 border-b-2 border-slate-200 gap-4">
           <div className="flex items-center gap-2">
              <span className="text-lg md:text-xl font-black uppercase italic tracking-tighter" style={{color:'#4c1d95'}}>Shining Stars</span>
              <span className="text-xl" style={{color:'#FFD700'}}>★</span>
           </div>
           <div className="flex flex-wrap items-center justify-center gap-2 md:gap-3">
             <button onClick={() => router.push('/staff')} className="bg-slate-200 text-slate-700 md:text-[10px] font-bold tracking-widest uppercase px-4 py-2 md:py-2.5 rounded-full hover:bg-slate-300 transition-all flex items-center gap-1" style={{fontSize:'9px'}}><span>⌂</span> HUB</button>
             <button onClick={() => router.push('/staff/student-dir')} className="text-white md:text-[10px] font-bold tracking-widest uppercase px-4 py-2 md:py-2.5 rounded-full hover:bg-black shadow-md" style={{background:'#4c1d95', fontSize:'9px'}}><span>◂</span> DIRECTORY</button>
             <button onClick={() => router.push('/')} className="bg-rose-500 text-white md:text-[10px] font-bold tracking-widest uppercase px-4 py-2 md:py-2.5 rounded-full shadow-md" style={{fontSize:'9px'}}>EXIT <span>⏏</span></button>
           </div>
        </div>
        ...
        <div className={`relative bg-gradient-to-br ${houseTheme.bg} p-6 md:p-10 rounded-[2rem] md:rounded-[3rem] shadow-2xl flex flex-col md:flex-row items-center md:items-start gap-6 overflow-hidden`}>
          ...
          <div className="z-10 mt-10 md:mt-0 flex-shrink-0 relative">
            <div className={`w-32 h-32 md:w-40 md:h-40 bg-white rounded-[1.5rem] md:rounded-[2rem] p-1.5 shadow-2xl overflow-hidden border-4 border-white/20 relative`}>
              {previewImg && <img src={previewImg} referrerPolicy="no-referrer" className="w-full h-full object-cover md:rounded-[1.7rem] relative z-10" style={{borderRadius:'1.2rem'}} onError={(e) => { e.target.style.display = 'none'; }} />}
          ...
          <div className="flex flex-col items-center md:items-start text-center md:text-left z-10 w-full min-w-0 md:mt-4">
            ...
            <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mt-4">
              <span className="text-white/90 font-bold uppercase md:text-[10px] bg-black/30 px-3 py-1.5 rounded-full border border-white/10" style={{fontSize:'9px'}}>ID: {student['Enrollment No.']}</span>
              <span className="text-white/90 font-bold uppercase md:text-[10px] bg-black/30 px-3 py-1.5 rounded-full border border-white/10" style={{fontSize:'9px'}}>GRADE: {student.Grade} {student.Class ? `• CLASS: ${student.Class}` : ''}</span>
        ...
        <div className="flex bg-white p-2 rounded-full shadow-sm border border-slate-200 gap-1 w-full max-w-md mx-auto relative -mt-4 z-20">
          {["PROFILE", "PERFORMANCE"].map(tab => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`flex-1 py-3 md:py-4 rounded-full font-black italic uppercase tracking-widest text-[10px] md:text-xs transition-all ${activeTab === tab ? 'bg-gold text-slate-950 shadow-md' : 'text-slate-400 hover:bg-slate-50'}`}>{tab}</button>
          ))}
        </div>
          ...
          {activeTab === "PROFILE" ? (
            <div className="space-y-6 md:space-y-8">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                <div className="bg-white p-6 md:p-8 border-2 border-slate-100 shadow-sm space-y-4" style={{borderRadius:'2rem'}}>
                   <h4 className="uppercase text-slate-400 font-bold tracking-widest border-b border-slate-100 pb-2" style={{fontSize:'10px'}}>ဖခင် အချက်အလက်</h4>
                   <div className="space-y-3">
                     ...
                     <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                        ...
                        <div className="flex flex-col flex-1"><span className="uppercase text-slate-400 font-bold" style={{fontSize:'8px'}}>အလုပ်အကိုင်</span><span className="text-sm md:text-base font-bold text-slate-700">{student["Father's Occupation"] || "—"}</span></div>
                     </div>
                   </div>
                </div>
                <div className="bg-white p-6 md:p-8 border-2 border-slate-100 shadow-sm space-y-4" style={{borderRadius:'2rem'}}>
                   <h4 className="uppercase text-slate-400 font-bold tracking-widest border-b border-slate-100 pb-2" style={{fontSize:'10px'}}>မိခင် အချက်အလက်</h4>
                   <div className="space-y-3">
                     ...
                     <div className="flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-6">
                        ...
                        <div className="flex flex-col flex-1"><span className="uppercase text-slate-400 font-bold" style={{fontSize:'8px'}}>အလုပ်အကိုင်</span><span className="text-sm md:text-base font-bold text-slate-700">{student["Mother's Occupation"] || "—"}</span></div>
                     </div>
                   </div>
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                <div className="bg-white p-6 md:p-8 border-2 border-slate-100 flex flex-col justify-center" style={{borderRadius:'2rem'}}><h4 className="uppercase text-slate-400 font-bold tracking-widest border-b border-slate-100 pb-2 mb-3" style={{fontSize:'10px'}}>အုပ်ထိန်းသူ အချက်အလက်</h4><p className="text-lg md:text-xl font-black text-slate-900 mb-3">{student["Guardian's Name"] || "—"}</p><PhoneLink label="အုပ်ထိန်းသူဖုန်း" phoneStr={student["Guardian's Phone"]} /></div>
                <div className="lg:col-span-2 bg-slate-50 p-6 md:p-8 border-2 border-slate-200 flex flex-col justify-center" style={{borderRadius:'2rem'}}><p className="uppercase text-slate-400 font-bold mb-3 italic" style={{fontSize:'10px'}}>နေရပ်လိပ်စာ (RESIDENTIAL ADDRESS)</p><p className="text-lg md:text-2xl font-black text-slate-900 italic break-words leading-relaxed">{[student["Address & Street"], student["Ward/Quarter"], student["Town"] || student["မြို့"]].filter(Boolean).join(", ") || "—"}</p></div>
                <div className="bg-white p-6 md:p-8 border-2 border-slate-100 flex flex-col items-center text-center" style={{borderRadius:'2rem'}}><h4 className="uppercase text-slate-400 font-bold tracking-widest mb-3 w-full border-b text-left" style={{fontSize:'10px'}}>ကျောင်းသား ဆက်သွယ်ရန်</h4><PhoneLink label="ကိုယ်ပိုင်ဖုန်း" phoneStr={student["Student's Phone"]} align="center" /></div>
                <div className="bg-indigo-50/50 p-6 md:p-8 border-2 border-indigo-100 text-center" style={{borderRadius:'2rem'}}><div className="w-12 h-12 bg-indigo-100 text-indigo-500 rounded-full flex items-center justify-center mx-auto mb-3">🏫</div><p className="uppercase text-indigo-400 font-bold mb-1 italic" style={{fontSize:'10px'}}>နေထိုင်မှုအမျိုးအစား</p><p className="text-xl md:text-2xl font-black text-indigo-900 uppercase">{student["School/Hostel"] || "—"}</p></div>
                <div className="bg-sky-50/50 p-6 md:p-8 border-2 border-sky-100 text-center" style={{borderRadius:'2rem'}}><div className="w-12 h-12 bg-sky-100 text-sky-500 rounded-full flex items-center justify-center mx-auto mb-3">🎓</div><p className="uppercase text-sky-400 font-bold mb-1 italic" style={{fontSize:'10px'}}>ပြောင်းလာခဲ့သော ကျောင်း</p><p className="text-base md:text-lg font-black text-sky-900 uppercase">{student["Transferred from"] || "—"}</p></div>
              </div>
            </div>
          ) : (
             <div className="space-y-6 md:space-y-8">
               ...
               <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 md:gap-8">
                 ...
                 <div className="bg-white p-6 md:p-10 border-2 border-slate-100 shadow-xl flex flex-col lg:col-span-2" style={{borderRadius:'2.5rem'}}>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="bg-slate-50 rounded-3xl border border-slate-200 overflow-hidden">
                        <div className="bg-slate-100 px-6 py-3 border-b border-slate-200 flex justify-between font-black uppercase text-slate-500 tracking-widest" style={{fontSize:'10px'}}>
                          ...
                          {allData.fees.length > 0 ? allData.fees.map((f, i) => (
                            <div key={i} className="px-6 py-4 flex justify-between items-center bg-white/50">
                              <div className="min-w-0">
                                <p className="font-black text-slate-400 italic" style={{fontSize:'10px'}}>{f.Date}</p>
                                <p className="text-sm font-bold text-slate-800 uppercase truncate">{f.Fee_Category}</p>
                              </div>
                              <p className="text-lg font-black text-emerald-600 shrink-0">{Number(f.Amount_Paid || 0).toLocaleString()} <span className="" style={{fontSize:'10px'}}>MMK</span></p>
                            </div>
                      ...
                      {currentDueAmount > 0 ? (
                        <div className="bg-rose-50 p-6 md:p-8 rounded-3xl border-2 border-rose-200 flex flex-col md:flex-row justify-between items-center gap-4 animate-in fade-in slide-in-from-bottom-2">
                          <div className="text-center md:text-left">
                            <p className="font-black uppercase text-rose-400 tracking-widest mb-1" style={{fontSize:'10px'}}>Current Outstanding Due (ပေးရန်ကျန်ငွေ)</p>
                          ...
                          <div className="bg-white/60 px-6 py-3 rounded-2xl border border-rose-100 text-center">
                            <p className="font-black uppercase text-rose-400" style={{fontSize:'9px'}}>Next Payment Due</p>
                 ...
                 <div className="bg-white p-6 md:p-8 border-2 border-slate-100 shadow-sm flex flex-col" style={{borderRadius:'2rem'}}>
                    ...
                    <div className="space-y-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                       {allData.notes.length > 0 ? [...allData.notes].reverse().map((n, i) => (
                         <div key={i} className="bg-slate-50 p-5 border border-slate-100" style={{borderRadius:'1.5rem'}}>
                           <p className="text-base md:text-lg font-bold italic text-slate-900">"{n.Note_Detail}"</p>
                           <div className="flex justify-between items-center font-bold uppercase text-slate-500 mt-3 pt-3 border-t border-slate-200" style={{fontSize:'9px'}}>
                 ...
                 <div className="bg-white p-6 md:p-8 border-2 border-slate-100 shadow-sm flex flex-col" style={{borderRadius:'2rem'}}>
                    ...
                    <div className="grid grid-cols-1 gap-3 max-h-[400px] overflow-y-auto pr-2 custom-scrollbar">
                       {allData.scores.length > 0 ? [...allData.scores].reverse().map((sc, i) => (
                         <div key={i} className="bg-slate-50 p-5 flex justify-between items-center border border-slate-100 hover:border-indigo-300 transition-all min-w-0" style={{borderRadius:'1.5rem'}}>
                            <div className="min-w-0 pr-2">
                               <p className="font-bold uppercase text-slate-400 mb-1 truncate" style={{fontSize:'9px'}}>{sc.Subject}</p>
                            ...
                            <div className="text-right flex-shrink-0">
                               ...
                               <p className="font-bold uppercase text-slate-400 mt-1" style={{fontSize:'9px'}}>Res: {sc.Result}</p>
                            </div>
                         </div>
...
function PhoneLink({ label, phoneStr, align = "left" }) {
  ...
  if (!phoneStr || phoneStr === "—") return (
    <div className={`flex flex-col min-w-0 flex-1 ${align === "center" ? "items-center text-center" : "items-start text-left"}`}>
      <span className="uppercase text-slate-400 font-bold tracking-widest" style={{fontSize:'8px'}}>{label}</span>
...
function StatCard({ label, value, unit, color }) {
  return (
    <div className="bg-white p-6 md:p-8 border-2 border-slate-100 shadow-sm flex flex-col justify-center min-w-0" style={{borderRadius:'2rem'}}>
      <p className="md:text-[10px] font-bold uppercase tracking-widest text-slate-400 mb-2 truncate" style={{fontSize:'9px'}}>{label}</p>
      <p className={`text-3xl md:text-5xl font-black font-serif-numbers italic tracking-tighter ${color} truncate`}>{value}</p>
      <p className="md:text-[9px] font-bold uppercase text-slate-300 italic tracking-widest mt-2 truncate" style={{fontSize:'8px'}}>{unit}</p>
    </div>
  );
}