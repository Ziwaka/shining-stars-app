...
export default function ManagementVendorsDirectory() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const payload = { action: 'saveVendor', userRole:'management', ...form, Updated_By: user.Name || user.username };
  ...
  const handleDelete = async (vid) => {
    ...
    setLoading(true);
    try {
      const res = await fetch(WEB_APP_URL, { method: 'POST', headers: {'Content-Type': 'text/plain;charset=utf-8'}, body: JSON.stringify({ action: 'deleteVendor', userRole:'management', Vendor_ID: vid }) });
  ...
  return (
    <div className="h-screen flex flex-col bg-[#F8FAFC] font-black text-slate-900 overflow-hidden">
      <div className="shrink-0 bg-white border-b border-slate-200 px-6 py-4 flex items-center justify-between z-10 shadow-sm">
        <button onClick={() => router.push('/management/mgt-dashboard')} className="w-10 h-10 rounded-full bg-slate-100 flex items-center justify-center text-slate-600 hover:bg-slate-200 transition-colors">←</button>
        <div className="text-center">
          <h1 className="text-slate-900 text-lg uppercase tracking-widest">Partners & Vendors</h1>
          <p className="text-sky-600 text-[10px] uppercase tracking-[0.2em]">Management Zone</p>
        </div>
