...
export default function ManagementVehiclesDashboard() {
  ...
  useEffect(() => {
    ...
    if (!u || u.userRole !== 'management') { router.push('/login'); return; }
    setUser(u);
    fetchData();
  }, []);
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      if (initRes.success) {
        ...
        // Enrich vehicles with Gender and Grade
        if (vRes.success) {
          const enriched = (vRes.data || []).map(v => {
             if(v.User_Type === 'STUDENT') {
               const s = st.find(x => x['Enrollment No.'] === v.User_ID || x.Student_ID === v.User_ID);
               return { ...v, Grade: s?.Grade || 'Unknown', Sex: s?.Sex || 'Unknown' };
             }
  ...
  const filteredPersons = personSearch.length >= 2 ? personList.filter(s => {
    const n = (s['Name (ALL CAPITAL)'] || s.Name || '').toLowerCase(), id = (s['Enrollment No.'] || s.Staff_ID || '').toString().toLowerCase(), query = personSearch.toLowerCase();
  ...
  const vStats = vehicles.reduce((acc, v) => {
    ...
    if (String(v.User_Type).toUpperCase() === 'STUDENT') {
      ...
      const g = v.Grade !== 'Unknown' ? `G-${v.Grade}` : 'Unknown';
  ...
  return (
    <div className="h-screen flex flex-col bg-[#F0F9FF] font-black text-slate-900 overflow-hidden">
      <div className="shrink-0 bg-slate-950 border-b-[8px] border-[#fbbf24] px-6 py-4 flex items-center justify-between z-10 shadow-xl">
        <button onClick={() => router.push('/management/mgt-dashboard')} className="bg-[#fbbf24] text-slate-950 w-10 h-10 rounded-xl flex items-center justify-center text-xl font-bold shadow-md hover:bg-amber-300">←</button>
        <div className="text-center">
          <h1 className="text-white text-lg uppercase tracking-widest italic">Vehicle Monitoring</h1>
          <p className="text-[#fbbf24] text-[9px] uppercase tracking-[0.2em]">Management Zone</p>
        </div>
        ...
        <div className="max-w-[1000px] mx-auto space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
             <div className="bg-slate-950 text-white rounded-3xl p-5 border-b-[6px] border-[#fbbf24] shadow-xl flex items-center gap-4 lg:col-span-1">
               ...
               <div><p className="text-3xl text-[#fbbf24] leading-none">{vStats.total}</p><p className="text-[9px] text-slate-400 uppercase tracking-widest mt-1">Total Registered</p></div>
             </div>
             ...
             <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col justify-center">
               ...
               <div className="flex justify-between items-center text-sm">
                 <span className="text-indigo-600 bg-indigo-50 px-2 py-1 rounded">🎓 Student: {vStats.student}</span>
                 <span className="text-amber-600 bg-amber-50 px-2 py-1 rounded">👔 Staff: {vStats.staff}</span>
               </div>
             </div>
             ...
             <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-col justify-center">
               ...
               <div className="flex justify-between items-center text-sm">
                 <span className="text-sky-600 bg-sky-50 px-2 py-1 rounded">♂️ Male: {vStats.male}</span>
                 <span className="text-pink-600 bg-pink-50 px-2 py-1 rounded">♀️ Female: {vStats.female}</span>
               </div>
             </div>

             <div className="bg-white rounded-3xl p-5 border border-slate-200 shadow-sm flex flex-wrap gap-2 justify-center items-center lg:col-span-1">
                {Object.entries(vStats.types).map(([type, count]) => (
                  <div key={type} className="text-center px-2 py-1 bg-slate-50 rounded-lg border border-slate-100 min-w-[60px]">
                    <p className="text-lg text-slate-800 leading-tight">{count}</p>
          ...
          <div className="bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
            <p className="text-[9px] text-slate-400 uppercase tracking-widest mb-4">Vehicles By Grade (Students)</p>
            <div className="flex items-end gap-2 h-24 overflow-x-auto pb-2">
              {Object.keys(vStats.grades).length === 0 ? <p className="text-xs text-slate-400 italic">No student vehicle records</p> : 
                Object.entries(vStats.grades).sort().map(([g, c]) => (
                <div key={g} className="flex-1 flex flex-col items-center justify-end min-w-[40px] group relative">
                   <div className="opacity-0 group-hover:opacity-100 absolute bottom-full mb-1 bg-slate-800 text-white text-[9px] px-2 py-1 rounded">{c}</div>
                   <div className="w-full max-w-[24px] bg-indigo-500 rounded-t-md transition-all" style={{height: `${Math.max((c/maxGradeVal)*100, 10)}%`}}></div>
                   <p className="text-[8px] text-slate-500 mt-2 rotate-45">{g}</p>
                </div>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-white p-4 rounded-3xl border border-slate-200 shadow-sm">
             <div className="flex gap-2 w-full md:w-auto">
               {['ALL', 'STUDENT', 'STAFF'].map(t => (
                 <button key={t} onClick={() => setFilterType(t)} className={`flex-1 md:flex-none px-4 py-2.5 rounded-xl text-[10px] uppercase tracking-widest transition-all ${filterType === t ? 'bg-[#fbbf24] text-slate-950 shadow-md' : 'bg-slate-100 text-slate-500'}`}>{t}</button>
               ))}
             </div>
             <div className="flex gap-2 w-full md:w-auto">
               <input placeholder="Search Name or Plate..." value={search} onChange={e => setSearch(e.target.value)} className="flex-1 px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm outline-none focus:border-[#fbbf24]" />
               <button onClick={openNew} className="shrink-0 bg-slate-950 text-white px-5 py-2.5 rounded-xl text-xs uppercase tracking-widest shadow-md hover:bg-slate-800">+ Register</button>
             </div>
          </div>

          <div className="space-y-4">
             {loading ? <div className="text-center py-20 text-slate-400 animate-pulse text-sm uppercase tracking-widest">Loading Records...</div> : displayVehicles.length === 0 ? (
                   ...
                   <div key={i} className="bg-white p-5 rounded-[2rem] border-b-[6px] border-slate-200 shadow-md hover:border-[#fbbf24] transition-all flex gap-5 items-center">
                      ...
                      <div className="flex-1 min-w-0">
                         <div className="flex justify-between items-start mb-1">
                           ...
                           <span className={`text-[8px] uppercase px-2 py-0.5 rounded-md font-bold ${v.User_Type==='STUDENT'?'bg-indigo-100 text-indigo-700':'bg-amber-100 text-amber-700'}`}>
                             {v.User_Type} {v.User_Type==='STUDENT' ? ` (${v.Grade})` : ''}
                           </span>
                         </div>
             ...
             <div className="overflow-y-auto flex-1 pr-2 space-y-5 pb-4">
               {!form.Reg_ID && !selectedPerson && (
                 <div className="space-y-3">
                   ...
                   <input placeholder={`${targetType} Name or ID...`} value={personSearch} onChange={e => setPersonSearch(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-4 py-3 text-sm outline-none focus:border-[#fbbf24]" />
                   {filteredPersons.map((p, i) => (
                     <button key={i} onClick={() => setSelectedPerson(p)} className="w-full text-left bg-white border border-slate-200 p-3 rounded-xl hover:border-[#fbbf24] flex justify-between items-center">
                       <div><p className="text-sm">{p['Name (ALL CAPITAL)'] || p.Name}</p><p className="text-[9px] text-slate-400 uppercase mt-1">ID: {p['Enrollment No.'] || p.Staff_ID}</p></div>
               ...
               {selectedPerson && (
                 <>
                   <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 flex justify-between items-center">
                     <div><p className="text-xs uppercase text-emerald-800">{selectedPerson['Name (ALL CAPITAL)'] || selectedPerson.Name}</p><p className="text-[9px] text-emerald-600 uppercase mt-1">ID: {selectedPerson['Enrollment No.'] || selectedPerson.Staff_ID} ({targetType})</p></div>
