...
import UserDetailModal from '@/features/leave/components/UserDetailModal';
...
export default function ManagementDashboard() {
  ...
  // Sub-tab for student roster: school / hostel
  const [rosterType, setRosterType] = useState('school');
  ...
  // Detailed student roster breakdown (by grade, section, gender) for school / hostel
  const rosterData = useMemo(() => {
    const school = new Map(); // grade -> section -> {male, female, total}
    const hostel = new Map();
    activeStudents.forEach(s => {
      const hostelVal = s['School/Hostel'] || s['School_Hostel'] || '';
      const isHostel = hostelVal.toString().toLowerCase() === 'hostel';
      const type = isHostel ? 'hostel' : 'school';
      const grade = s.Grade || 'Unknown';
      const section = s.Section || s.section || '-';
      const sex = (s.Sex || '').toString().toUpperCase();
      const isMale = sex === 'M' || sex === 'MALE' || sex === 'ကျား';
      const map = isHostel ? hostel : school;
      if (!map.has(grade)) map.set(grade, new Map());
      const secMap = map.get(grade);
      if (!secMap.has(section)) secMap.set(section, { male: 0, female: 0, total: 0 });
      const entry = secMap.get(section);
      if (isMale) entry.male++;
      else entry.female++;
      entry.total++;
    });
    ...
    const schoolGrades = Array.from(school.keys()).sort(sortGrades);
    const hostelGrades = Array.from(hostel.keys()).sort(sortGrades);

    const schoolTotals = new Map();
    schoolGrades.forEach(g => {
      let m=0, f=0;
      const secs = school.get(g);
      for (let [_, v] of secs) { m += v.male; f += v.female; }
      schoolTotals.set(g, { male: m, female: f, total: m+f });
    });
    const hostelTotals = new Map();
    hostelGrades.forEach(g => {
      let m=0, f=0;
      const secs = hostel.get(g);
      for (let [_, v] of secs) { m += v.male; f += v.female; }
      hostelTotals.set(g, { male: m, female: f, total: m+f });
    });

    return {
      school: { grades: schoolGrades, map: school, totals: schoolTotals },
      hostel: { grades: hostelGrades, map: hostel, totals: hostelTotals }
    };
  }, [activeStudents]);
  ...
  const renderRosterTable = (type) => {
    const data = rosterData[type];
    if (!data || data.grades.length === 0) return (
      <p style={{ fontSize:11, color:'#94A3B8', textAlign:'center', padding:16 }}>
        No active {type} students
      </p>
    );
      ...
      <table style={{ width:'100%', borderCollapse:'collapse', fontSize:10 }}>
        <thead>
          <tr style={{ borderBottom:'1px solid #E2E8F0', color:'#64748B' }}>
            <th style={{ textAlign:'left', padding:'6px 8px', fontWeight:700 }}>Grade</th>
            <th style={{ textAlign:'left', padding:'6px 8px', fontWeight:700 }}>Section</th>
            <th style={{ textAlign:'right', padding:'6px 8px', fontWeight:700 }}>Male</th>
            <th style={{ textAlign:'right', padding:'6px 8px', fontWeight:700 }}>Female</th>
            <th style={{ textAlign:'right', padding:'6px 8px', fontWeight:700 }}>Total</th>
          </tr>
        </thead>
        <tbody>
          {data.grades.map(grade => {
            const sections = data.map.get(grade);
            const gradeTotal = data.totals.get(grade);
            const rows = [];
            for (let [sec, counts] of sections) {
              rows.push(
                <tr key={`${grade}-${sec}`} style={{ borderBottom:'1px solid #F1F5F9' }}>
                  <td style={{ padding:'6px 8px', fontWeight:500, color:'#1E293B' }}>{grade}</td>
                  <td style={{ padding:'6px 8px', color:'#475569' }}>{sec}</td>
                  <td style={{ padding:'6px 8px', textAlign:'right', color:'#2563EB' }}>{counts.male}</td>
                  <td style={{ padding:'6px 8px', textAlign:'right', color:'#DB2777' }}>{counts.female}</td>
                  <td style={{ padding:'6px 8px', textAlign:'right', fontWeight:600, color:'#0F172A' }}>{counts.total}</td>
                </tr>
              );
            }
            rows.push(
              <tr key={`${grade}-total`} style={{ background:'#F8FAFC', borderBottom:'1px solid #E2E8F0' }}>
                <td style={{ padding:'6px 8px', fontWeight:700, color:'#0F172A' }} colSpan={2}>
                  Grade {grade} Total
                </td>
                <td style={{ padding:'6px 8px', textAlign:'right', fontWeight:700, color:'#1D4ED8' }}>{gradeTotal.male}</td>
                <td style={{ padding:'6px 8px', textAlign:'right', fontWeight:700, color:'#BE185D' }}>{gradeTotal.female}</td>
                <td style={{ padding:'6px 8px', textAlign:'right', fontWeight:700, color:'#0F172A' }}>{gradeTotal.total}</td>
              </tr>
            );
            return rows;
          })}
          {(() => {
            ...
            data.grades.forEach(g => {
              const t = data.totals.get(g);
              gm += t.male;
              gf += t.female;
            });
            return (
              <tr style={{ background:'#FFF7ED', borderTop:'2px solid #F59E0B' }}>
                <td style={{ padding:'8px', fontWeight:900, color:'#0F172A' }} colSpan={2}>
                  Grand Total ({type === 'school' ? 'School' : 'Hostel'})
                </td>
                <td style={{ padding:'8px', textAlign:'right', fontWeight:900, color:'#1D4ED8' }}>{gm}</td>
                <td style={{ padding:'8px', textAlign:'right', fontWeight:900, color:'#BE185D' }}>{gf}</td>
                <td style={{ padding:'8px', textAlign:'right', fontWeight:900, color:'#0F172A' }}>{gm+gf}</td>
              </tr>
            );
          })()}
        </tbody>
    ...
    <>
      ...
      <div style={{ flex:1, overflowY:'auto', background:'#F5F3EE', WebkitOverflowScrolling:'touch', paddingBottom:'40px' }}>
        ...
        <div style={{ maxWidth:500, margin:'0 auto', padding:'16px 16px 0' }}>
          <div style={{ display:'flex', gap:0, background:'#E2E8F0', borderRadius:12, padding:4 }}>
            <button
              ...
              style={{
                ...
                boxShadow: activeTab === 'dashboard' ? '0 1px 4px rgba(0,0,0,0.08)' : 'none',
                transition:'all 0.2s'
              }}>
              📊 Dashboard
            </button>
            <button
              ...
              style={{
                ...
                boxShadow: activeTab === 'tools' ? '0 1px 4px rgba(0,0,0,0.08)' : 'none',
                transition:'all 0.2s'
              }}>
              ⚡ Tools
            </button>
        ...
        {activeTab === 'dashboard' ? (
          <div style={{ fontFamily:"'DM Sans',system-ui,sans-serif" }}>
            {/* ====== HEADER (inside Dashboard tab only) ====== */}
            ...
            {/* Dashboard tab lower content (Attendance, Leaves, Roster) */}
            <div style={{ maxWidth:500, margin:'0 auto', padding:'16px', display:'flex', flexDirection:'column', gap:18 }}>
              ...
              {/* Student Roster Summary (School / Hostel breakdown) */}
              <div style={{ borderRadius:18, overflow:'hidden',
                            boxShadow:'0 4px 16px rgba(0,0,0,0.09)', animation:'fadeUp 0.35s ease 0.25s both' }}>
                <div style={{ background:'#fff', padding:'16px' }}>
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:14 }}>
                    ...
                    <div style={{ display:'flex', gap:0, background:'#F1F5F9', borderRadius:8, padding:3 }}>
                      <button
                        onClick={() => setRosterType('school')}
                        style={{
                          padding:'6px 14px', border:'none', borderRadius:6,
                          background: rosterType === 'school' ? '#fff' : 'transparent',
                          fontWeight:600, fontSize:11, cursor:'pointer',
                          color: rosterType === 'school' ? '#0F172A' : '#64748B',
                          boxShadow: rosterType === 'school' ? '0 1px 4px rgba(0,0,0,0.06)' : 'none',
                        }}>
                        🏫 School
                      </button>
                      <button
                        onClick={() => setRosterType('hostel')}
                        style={{
                          padding:'6px 14px', border:'none', borderRadius:6,
                          background: rosterType === 'hostel' ? '#fff' : 'transparent',
                          fontWeight:600, fontSize:11, cursor:'pointer',
                          color: rosterType === 'hostel' ? '#0F172A' : '#64748B',
                          boxShadow: rosterType === 'hostel' ? '0 1px 4px rgba(0,0,0,0.06)' : 'none',
                        }}>
                        🏠 Hostel
                      </button>
                    </div>
          ...
          <div style={{ maxWidth:500, margin:'0 auto', padding:'16px',
            ...
            {TOOL_SECTIONS.map((sec, si) => (
              <div key={si} style={{ animation:`fadeUp 0.35s ease ${0.1+si*0.06}s both` }}>
